...
class EventChatScreen extends Component {
    ...
    runningFetchMessage = false; source = '';
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    _readMsgs = async () => {
        Axios.get(`${SERVER_URL}?action=readMsg&chat_id=${this.state.event_id}&isgrp=1&userId=${this.state.userID}`)
            .then(response => {
            })
            .catch(err => {
    ...
    getMessages() {
        if (this.runningFetchMessage == false) {
            ...
            Axios.get(`${SERVER_URL}?action=getEventMessages&event_id=${this.state.event_id}`, { cancelToken: this.source.token })
                .then((res) => {
                    ...
                    if (this._isMounted) {
                        ...
                        this.setState({ isloadingMsgs: false }, () => { this.props.loadingChangeAction(false); });
    ...
    */
    sendNewMessage() {
        ...
        if (message) {
            ...
            Axios.get(`${SERVER_URL}?action=msgSend&user_id=${this.state.userID}&grp_id=${this.state.event_id}&is_grp_msg=1&msg_text=${message}`)
                .then(res => { })
                .catch(err => {
    ...
    */
    _fetchUserData = user_id => {
        ...
        Axios.get(`${SERVER_URL}?action=get_user_data&user_id=${user_id}`)
            ...
            .catch(err => {
                Toast.show(err.message, Toast.SHORT);
                this.setState({ isLoadingProfile: false, profileDetailShow: false });
            });
    };
    render() {
...
export default connect(mapStateToProps, mapDispatchToProps)(EventChatScreen);